...
void tfmxaudiodecoder::TFMXDecoder::traitsByChecksum() {
    ...
    // Turrican II loader jingle exists as a repaired version
    // (with crc1 = 0xc1ac5e1c) and as an "unfixed" version that
    // sounds bad. The repaired version modified the first two
    // macro scripts. What's the full story there?!
    else if (crc1 == 0xcbb842b0) {
        const ubyte m0[] = {
            0x00,0x01,0x00,0x00, 0x29,0x01,0x00,0x00,
            0x22,0x00,0x04,0x14, 0x23,0x20,0x00,0x0f,
            0x24,0x00,0x00,0x8b, 0x25,0x00,0xbf,0x50,
            0x09,0x0a,0x00,0x00, 0x01,0x01,0x00,0x00,
            0x0e,0x00,0x00,0x40, 0x04,0x00,0x00,0x00,
            0x29,0x00,0x00,0x00, 0x19,0x00,0x00,0x00,
            0x04,0x00,0x00,0x00, 0x07,0x00,0x00,0x00
        };
        const ubyte m1[] = {
            0x00,0x01,0x00,0x00, 0x29,0x01,0x00,0x00,
            0x22,0x00,0x04,0x14, 0x23,0x50,0x00,0x0f,
            0x24,0x00,0x00,0x87, 0x25,0x00,0xbf,0x20,
            0x26,0x0f,0xff,0xf0, 0x09,0x0f,0x00,0x00,
            0x01,0x01,0x00,0x00, 0x0e,0x00,0x00,0x40
        };
        for (int i=0; i<sizeof(m0); ++i) {
            pBuf[offsets.header+0x300+i] = m0[i];
        }
        for (int i=0; i<sizeof(m1); ++i) {
            pBuf[offsets.header+0x338+i] = m1[i];
        }
    }
}
