...
void tfmxdec_set_path(void* decoder, const char* path) {
    TFMX_DECLARE_DECODER;
    p->dec.setPath(path);
}
...
int tfmxdec_detect(void* decoder, void* buffer, uint32_t length) {
    TFMX_DECLARE_DECODER;
    return p->dec.maybeOurs(buffer,length);
}

int tfmxdec_init(void* decoder, void* buffer, uint32_t length, int songNumber) {
    TFMX_DECLARE_DECODER;
    return p->dec.init(buffer,length,songNumber);
}
...
void tfmxdec_mixer_init(void* decoder, int frequency, int precision, int channels, int zero, int panning) {
    TFMX_DECLARE_DECODER;
    p->mixer.init(frequency,precision,channels,zero,panning);
}
...
uint16_t tfmxdec_get_voice_volume(void* decoder, unsigned int voice) {
    TFMX_DECLARE_DECODER;
